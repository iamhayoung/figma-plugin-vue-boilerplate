...
interface VariablesAPI {
  ...
  setBoundVariableForPaint(paint: SolidPaint, field: VariableBindablePaintField, variable: Variable): SolidPaint;
  setBoundVariableForEffect(effect: Effect, field: VariableBindableEffectField, variable: Variable): Effect;
...
interface CodegenAPI {
  on(type: 'generate', callback: (event: CodegenEvent) => Promise<CodegenResult[]> | CodegenResult[]): void;
  on(type: 'preferenceschange', callback: (event: CodegenPreferencesEvent) => Promise<void>): void;
  once(type: 'generate', callback: (event: CodegenEvent) => Promise<CodegenResult[]> | CodegenResult[]): void;
  once(type: 'preferenceschange', callback: (event: CodegenPreferencesEvent) => Promise<void>): void;
  off(type: 'generate', callback: (event: CodegenEvent) => Promise<CodegenResult[]> | CodegenResult[]): void;
...
interface DevResourcesAPI {
  on(type: 'linkpreview', callback: (event: LinkPreviewEvent) => Promise<LinkPreviewResult> | LinkPreviewResult): void;
  ...
  off(type: 'linkpreview', callback: (event: LinkPreviewEvent) => Promise<LinkPreviewResult> | LinkPreviewResult): void;
...
declare type TextCase = 'ORIGINAL' | 'UPPER' | 'LOWER' | 'TITLE' | 'SMALL_CAPS' | 'SMALL_CAPS_FORCED';
...
declare type VariableDataType = 'BOOLEAN' | 'FLOAT' | 'STRING' | 'VARIABLE_ALIAS' | 'COLOR' | 'EXPRESSION';
...
interface OpaqueNodeMixin extends BaseNodeMixin, SceneNodeMixin, ExportMixin, DimensionAndPositionMixin {}
...
interface NonResizableTextMixin {
  ...
  getStyledTextSegments<
    StyledTextSegmentFields extends (keyof Omit<StyledTextSegment, 'characters' | 'start' | 'end'>)[]
  >(
...
interface BooleanOperationNode extends DefaultShapeMixin, ChildrenMixin, CornerMixin, ContainerMixin {
...
interface HighlightNode extends DefaultShapeMixin, ConstraintMixin, CornerMixin, VectorLikeMixin, StickableMixin {
